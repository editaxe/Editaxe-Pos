...
$cod_plan_separe    = intval($_POST['cod_plan_separe']);
//---------------------------------------------------------------------------------------------------------------------------------//
//---------------------------------------------------------------------------------------------------------------------------------//
...
$obtener_calculo_fact = "SELECT sum(vlr_total_venta) as vlr_total_venta FROM plan_separe_producto WHERE cod_plan_separe = '$cod_plan_separe'";
...
Sum(vlr_total_venta*($descuento/100)) AS total_desc, Sum(vlr_total_venta) AS total_venta_neta FROM plan_separe_producto WHERE cod_plan_separe = '$cod_plan_separe'";
...
$printer->text("\n");
$printer->setEmphasis(true);
$printer->text("FACTURA PLAN SEPARE: ");
$printer->setEmphasis(false);
$printer->text(str_pad($cod_plan_separe, 5, "0", STR_PAD_LEFT));
$printer->text("\n");
$printer->setEmphasis(true);
...
$printer->text(str_pad("DESCRIPCION", 22));
...
FROM plan_separe_producto WHERE cod_plan_separe = '$cod_plan_separe'";
...
$printer->setJustification(Printer::JUSTIFY_LEFT);
$printer->text(str_pad($unidades_vendidas, 4));
...
//$printer->text("DESDE ".str_pad($res1_emp, 5, "0", STR_PAD_LEFT). " AL ".Str_pad($res2_emp, 5, "0", STR_PAD_LEFT)."\n");
...
$barra      = $estandar_b.str_pad($cod_plan_separe, 5, "0", STR_PAD_LEFT);
$printer -> setJustification(Printer::JUSTIFY_CENTER);
$printer -> setBarcodeHeight(80);
//$printer->setBarcodeTextPosition(Printer::BARCODE_TEXT_BELOW);
$printer -> barcode($barra, Printer::BARCODE_CODE128);
$printer->text("<-- ".$fecha.$hora."-".$cod_plan_separe."-".$fecha_venta_ymd.$hora_venta_his." -->");
//---------------------------------------------------------------------------------------------------------------------------------//
//---------------------------------------------------------------------------------------------------------------------------------//
