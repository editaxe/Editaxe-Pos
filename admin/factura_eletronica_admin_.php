...
include_once('class_php/numeros_a_letras_funcion.php');
...
<td id="nombre_productos<?php echo $cod_temporal;?>"><?php echo $nombre_productos;?></td>
<td id="unidades_vendidas<?php echo $cod_temporal;?>" align='center'><input onFocus="Focus(this.id, this.value)" onChange="calc_total_venta();" onBlur="Blur(this.id, this.value, 'unidades_vendidas', <?php echo $cod_temporal;?>)" class="cajund" id="unidades_vendidas<?php echo $incre;?>" value="<?php echo $unidades_vendidas;?>" size="3"></td>
<td id="mensaje_alerta<?php echo $incre;?>"></td>

<?php if ($detalles == 'PV1') { ?> 
<td id="tipo_precio_venta<?php echo $cod_temporal;?>" align='center'><a href="../admin/actualizar_precio_venta.php?tipo_precio_venta=PV2&cod_temporal=<?php echo $cod_temporal?>&pagina=<?php echo $pagina?>"><img src="../imagenes/PV1.png" alt="Precio venta"></a></td> 
<?php } 
elseif ($detalles == 'PV2') {?>
<td id="tipo_precio_venta<?php echo $cod_temporal;?>" align='center'><a href="../admin/actualizar_precio_venta.php?tipo_precio_venta=PV3&cod_temporal=<?php echo $cod_temporal?>&pagina=<?php echo $pagina?>"><img src="../imagenes/PV2.png" alt="Precio descuento"></a></td> 
<?php }
elseif ($detalles == 'PV3') {?>
<td id="tipo_precio_venta<?php echo $cod_temporal;?>" align='center'><a href="../admin/actualizar_precio_venta.php?tipo_precio_venta=PV1&cod_temporal=<?php echo $cod_temporal?>&pagina=<?php echo $pagina?>"><img src="../imagenes/PV3.png" alt="Precio por mayor"></a></td> 
<?php } ?>

<?php if ($detalles == 'PVAR') { ?>
<td id="PVAR<?php echo $cod_temporal;?>" align='center'><img src="../imagenes/PVAR.png" alt="Precio variable"></a></td> 
<td id="precio_venta<?php echo $cod_temporal;?>" align='center'><input onFocus="Focus(this.id, this.value)" onChange="calc_total_venta();" onBlur="Blur(this.id, this.value, 'precio_venta', <?php echo $cod_temporal;?>)" class="cajvtotal" id="precio_venta<?php echo $incre;?>" value="<?php echo $precio_venta;?>" size="3"></td>
<?php } 
else { ?>
<td id="precio_venta_no_modif<?php echo $cod_temporal;?>" style="text-align:center; font-size:30;" align='right'><?php echo number_format($precio_venta, 0, ",", ".");?></td>
<?php } ?>
    ...
    $('.eliminar').click(function(){
        ...
        $.ajax({
            ...
            success: function() {           
                ...
                $('#nombre_productos'+cod_temporal).fadeOut("slow");
                $('#unidades_vendidas'+cod_temporal).fadeOut("slow");
                $('#tipo_precio_venta'+cod_temporal).fadeOut("slow");
                $('#PVAR'+cod_temporal).fadeOut("slow");
                $('#precio_venta'+cod_temporal).fadeOut("slow");
                $('#precio_venta_no_modif'+cod_temporal).fadeOut("slow");
                $('#vlr_total_venta'+cod_temporal).fadeOut("slow");
