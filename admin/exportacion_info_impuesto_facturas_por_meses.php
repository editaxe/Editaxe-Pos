...
<br><br>
<td><font size="6px" color="yellow">EXPORTAR POR MES DE INFO FACTURA</font></td>
<br><br>
<table>
...
<?php $sql_consulta="SELECT distinct fecha_mes FROM info_impuesto_facturas ORDER BY fecha_dia ASC";
...
<?php $sql_consulta="SELECT distinct fecha_mes FROM info_impuesto_facturas ORDER BY fecha_dia DESC";
$resultado = mysql_query($sql_consulta, $conectar) or die(mysql_error());
while ($contenedor=mysql_fetch_array($resultado)) {?>
<option style="font-size:13px" value="<?php echo $contenedor['fecha_mes'] ?>"><?php echo $contenedor['fecha_mes'] ?></option>
<?php }?></select></td>
