...
//---------------------------------------------------------------------------------------------------------------------------------//
$suma_temporal = "SELECT Sum(vlr_total_venta - (vlr_total_venta*($descuento_factura/100))) As total_venta, Sum(vlr_total_compra) AS vlr_total_compra 
...
nombre_ccosto, cod_dependencia FROM temporal WHERE (cod_temporal = '$cod_temporal')";
...
//-------------------------------------------------------------------------------------------------------------//
$tiempo_final = microtime(true); $tiempo_ejecucion = $tiempo_final - $tiempo_inicial; //echo "El tiempo de ejecucion ".round($tiempo_ejecucion, 2)." segundos";
...
fecha_mes = '$fecha_mes', fecha_hora = '$fecha_hora', tiempo_ejecucion = '$tiempo_ejecucion' WHERE cod_info_impuesto_facturas = '$cod_info_impuesto_facturas'");
...
descuento = '$descuento_factura', tiempo_ejecucion = '$tiempo_ejecucion' WHERE cod_info_impuesto_facturas = '$cod_info_impuesto_facturas'");
$resultado_regis = mysql_query($agregar_regis, $conectar) or die(mysql_error());
}
//-------------------------------------------------------------------------------------------------------------//
if ($bolsa <> '0') {
...
$sql_mconsulta = "SELECT cod_productos, detalles, unidades_vendidas, precio_venta, nombre_productos, precio_compra, precio_costo, vlr_total_venta, 
vlr_total_compra, iva, descuento, precio_compra_con_descuento, vendedor, porcentaje_vendedor, nombre_lineas, nombre_ccosto, cod_dependencia 
FROM temporal WHERE (cod_temporal = '$cod_temporal')";
$mconsulta = mysql_query($sql_mconsulta, $conectar) or die(mysql_error());
$datos_temp = mysql_fetch_assoc($mconsulta);
...
fecha, fecha_invert) VALUES ('$cod_clientes', '$cod_factura', '$monto_deuda', '$subtotal', '$descuento_factura', '$vendedor', '$fecha_pago', '$fecha_anyo', '$fecha_invert')";
...
fecha_mes = '$fecha_mes', fecha_hora = '$fecha_hora', estado = '$estado', descuento = '$descuento_factura', tiempo_ejecucion = '$tiempo_ejecucion' 
