...
$mostrar_datos_sql = "SELECT * FROM entrada_personal WHERE fecha_dmy = '$fecha' order by fecha_seg DESC";
$consulta = mysql_query($mostrar_datos_sql, $conectar) or die(mysql_error());
$datos = mysql_fetch_assoc($consulta);

$datos_sql_salida = "SELECT SUM(costo) AS costo FROM salida_personal WHERE fecha_dmy = '$fecha' order by fecha_seg DESC";
...
$datos_sql_entrada = "SELECT SUM(costo) AS costo FROM entrada_personal WHERE fecha_dmy = '$fecha' order by fecha_seg DESC";
...
<?php $sql_consulta = "SELECT fecha_dmy FROM entrada_personal GROUP BY fecha_dmy ORDER BY fecha_seg DESC";
$resultado = mysql_query($sql_consulta, $conectar) or die(mysql_error());
while ($contenedor = mysql_fetch_assoc($resultado)) {?>
<option style="font-size:20px" value="<?php echo $contenedor['fecha_dmy'] ?>"><?php echo $contenedor['fecha_dmy']?></option>
<?php }?></select></td>
<tr valign="baseline">
