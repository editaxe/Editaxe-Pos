...
//Sentencia SQL
$sql = mysql_query("SELECT sum(vlr_total_venta) AS vlr_total_venta, fechas_vencimiento_seg FROM productos 
	GROUP BY fechas_vencimiento_seg ORDER BY fechas_vencimiento_seg ASC");
...
$time = date("Y/m/d", $vector_de_datos[$i]["fechas_vencimiento_seg"]);
$date = new DateTime($time);
$vector_de_datos[$i]["fechas_vencimiento_seg"]=$date->getTimestamp()*1000;
}
?> 
<HTML>
<BODY>
...
for($i = 0 ;$i<count($vector_de_datos);$i++){
?>
data.push([<?php echo $vector_de_datos[$i]["fechas_vencimiento_seg"];?>,<?php echo $vector_de_datos[$i]["vlr_total_venta"];?>]);
<?php } ?>
return data;
})()
},{            
